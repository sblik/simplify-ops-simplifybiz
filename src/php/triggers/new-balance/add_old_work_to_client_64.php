...
function bs_create_child_submissions_150_64() {
	SMPLFY_Log::info( "FUNCTION 64 TRIGGERED" );
	$ops_form_id                        = 50;
	 ...
	 */
	$clients_form_id                    = 150;
	$search_criteria['field_filters'][] = array( 'key' => '3', 'value' => 64 );
	$search_criteria['status'][]        = 'active';
	$client_entries_2                   = GFAPI::get_entries( $clients_form_id, $search_criteria, $sorting );
	SMPLFY_Log::info( $client_entries_2[0] );

	for ( $i = 0; $i < $count_ops_entries; $i ++ ) {
		SMPLFY_Log::info( "CLIENT USER ID: " . $ops_entries[ $i ][2] );

		if ( $ops_entries[ $i ][2] == 64 ) {
			SMPLFY_Log::info( $client_entries_2[0] );
			SMPLFY_Log::info( "CLIENT ENTRY USER ID: " . $client_entries_2[0][2] );
			...
			$new_child_entry = array(
				...
				'7'                                     => $workCompleted,
				'8'                                     => $hoursSpent,
				GPNF_Entry::ENTRY_PARENT_KEY            => $client_entries_2[0]['id'], // The ID of the parent entry.
